...
collision_insert[iteration] = mht.getNumOfCollision_insert();
mht.setCol_insert(); //setting the collision to 0 for the next iteration
...
//mht.printTable();
    ...
    for(j=0; j < 100; j++) 
    {
       ...
       tempNode = mht.searchItem(search_key[j]); 
       // cout << saveNode->key << " ";
    }
